  ...
  <div id="root" class="root hd--expanded hd--sticky mn--sticky" :class="{ 'mn--max' : !collapse, 'mn--min' : collapse, }">
        ...
        <div class="content__header content__boxed overlapping">
            <div class="content__wrap">
                ...
                <h1 class="page-title mb-2">Prodotto Composto (Bulk)</h1>
                ...
                <div class="row p-2 bg-light">
                    ...
                    <div class="col-4 row">
                        <div class="col-4">
                            <h5>Associato a:</h5>
                            <ul class="m-0 p-0">
                                <li v-for="(obj,a) in product.bulk_products_qty" :key="a">
                                    <b>{{obj.qty}}</b> di <a :href="'/product/simple/?id='+obj.product.id+'&marketplace='+this.marketplace.id+'&company='+this.company.id">{{obj.product.sku}}</a>
                                </li>
                            </ul>
                        </div>
                        <div class="col-6 m-auto">
                            <button class="btn btn-warning" v-on:click="this.changeAssociation()">Cambia associazione</button>
                        </div>
                    </div>
                    <div class="col-4 d-md-flex justify-content-md-end">
                        <a :href="'/products/bulk/?company='+this.company.id+'&marketplace='+this.marketplace.id" class="m-auto me-0 btn btn-primary">Tutti i prodotti</a>
                    </div>
                    <div class="row mt-3 mb-3" v-if="this.association">
                        <div class="card col-12 m-auto p-4">
                            <div class="card-header row">
                                <div class="col-3 mt-2">
                                    <h6 class="card-title">Stai modificando l'associazione (composto/semplici)</h6>
                                </div>
                                <div class="col-2 ms-0 m-auto">
                                    <div class="col-12 m-auto d-md-flex justify-content-md-end"><button class="btn btn-warning" v-on:click="this.addBulkChild()">Aggiungi un prodotto semplice</button></div>
                                </div>
                            </div>
                            ...
                            <div class="card-footer row">
                                ...
                                <button class="btn btn-warning" v-on:click="this.resetAssociation()">Annulla</button>
                                </div>
                                <div class="col-6 d-md-flex justify-content-md-end">
                                <button class="btn btn-warning" v-on:click="this.saveAssociation()">Salva</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default{
	...
	methods:{
        ...
        async changeAssociation(){
            ...
            for(let i=0;i<this.product.bulk_products_qty.length;i++){
                let obj={}
                obj["id"]=this.product.bulk_products_qty[i].product.id
                obj["qty"]=this.product.bulk_products_qty[i].qty
                await this.axios.get("/api/products/categories/?company="+this.company.id+"&marketplace="+this.marketplace.id+"&simple="+obj["id"]).then((res)=>{
                        if(res.data.results.length>0){
                            obj["category"]=res.data.results[0].id;
                        }
            ...
            for(let i=0;i<this.bulk.childs.length;i++){
                this.getAbstractSimpleProductsInCategoryForBulk(i)
            }
            
        },
        async resetAssociation(){
      ...
      const res = await this.axios.get("/api/simplify/products/categories/?company="+this.company.id+"&marketplace="+this.marketplace.id).then((res) => {
        ...
        async getProductCategory(){
            const res = await this.axios.get("/api/products/categories/?company="+this.company.id+"&marketplace="+this.marketplace.id+"&bulk="+this.$route.query.id).then((res)=>{
                if(res.data.results.length>0){
        ...
        async getProduct(){
                ...
                const res = await this.axios.get("/api/products/bulk/"+this.$route.query.id+"/?company="+this.company.id+"&marketplace="+this.marketplace.id).then((res)=>{
                        ...
                        this.product.bulk_products_qty=res.data.bulk_products_qty
        ...
        saveProduct(){
            ...
            this.axios.put("/api/products/bulk/"+this.product.id+"/?company="+this.company.id+"&marketplace="+this.marketplace.id,this.product).then((res)=>{
        ...
        copyToProducts(){
            ...
            this.axios.put("/api/copy/to/bulk/?company="+this.company.id+"&marketplace="+this.marketplace.id+"&product="+this.product.id,data).then((res)=>{
        ...
        copyFromProduct(){
            ...
            this.axios.put("/api/copy/from/bulk/?company="+this.company.id+"&marketplace="+this.marketplace.id+"&product="+this.product.id,data).then((res)=>{
                this.toast.success("Copia effettuata");                
            }).catch((error)=>{
                this.toast.error(error.response.data.detail)
            })
    ...
    async saveAssociation(){
      ...
      await this.axios.put("/api/products/bulk/"+this.product.id+"/?company="+this.company.id+"&marketplace="+this.marketplace.id,bulk).then((res) => {
          this.toast.success("Associazioni aggiornate");
          this.$router.go()
        })
        .catch((error) => {
          this.toast.error(error.response.data.detail);
