    ...
    <div id="root" class="root hd--expanded hd--sticky mn--sticky" :class="{ 'mn--max' : !collapse, 'mn--min' : collapse, }">
         ...
         <div class="content__header content__boxed overlapping">
             <div class="content__wrap">
                     ...
                     <h1 class="page-title mb-2">Prodotti Configurabili</h1>
                     <h2 class="h5">Lista delle schede dei prodotti configurabili</h2>
                     <p></p>
                             ...
                             <tbody>
                                 <tr v-for="(product,index) in products" :key="index">
                                     ...
                                     <td v-show="fields['child'].show">
                                        <div class="row">
                                            <div class="col-6">
                                                <ul class="p-0 m-0" v-if="'products' in product" v-for="(obj,id) in product.products" :key="id">
                                                    <li>
                                            ...
                                            <div class="col-6">
                                                <ul class="p-0 m-0" v-if="'variations' in product" v-for="(obj,id) in product.variations" :key="id">
                                                    <li>
                                                        {{obj.description}}<br/>
                                                    </li>
                                                </ul>
                                     ...
                                     <td><a class="btn" title="Vedi/Modifica" :href="'/product/configurable/?company='+this.company.id+'&marketplace='+this.marketplace.id+'&id='+product.id"><i class="bi bi-eye"></i></a></td>
                                 </tr>
                             </tbody>
                         </table>
                     </div>
         ...
         <Sidebar :collapse.sync="collapse" :company.sync="company" :companies.sync="companies" @update:company="(index) => changeCompany(index)" parent="products" child="configurable" @update:collapse="(collapse=false)"/>
     </div>
 </template>
 ...
 function initialState (){
   return {
             ...
             fields: {
                 ...
                 child:{name:"Prodotti/Variazioni",show:true},
                 brand:{name:"Brand",show:true},
     ...
     methods:{
         async getProductsCategories(){
             for(let i=0;i<this.products.length;i++){
                 await this.axios.get("/api/products/categories/?company="+this.company.id+"&marketplace="+this.marketplace.id+"&configurable="+this.products[i].id).then((res)=>{
         ...
         async getProducts() {
             ...
             try{
                 const res = await this.axios.get("/api/products/configurable/?company="+this.company.id+"&marketplace="+this.marketplace.id+"&offset="+this.offset+"&limit="+this.limit+"&search="+this.search).then((res) => {
                     this.products=[]
                     for(var i=0; i<res.data.results.length; i++){
                         if(res.data.next){
             ...
             if(this.actionToDo=="duplicate"){
                 ...
                 for(var [key,product] of Object.entries(this.products)){
                     if(id_list.includes(product.id)){
                         ...
                         data["childs"]=product.products
                         data["variations"]=product.variations
                         
                         
                         
                         await this.axios.post("/api/products/configurable/?company="+this.company.id+"&marketplace="+this.actionMarket,data).then((res)=>{
                 ...
                 for(var i=0;i<id_list.length;i++){
                     await this.axios.delete("/api/products/configurable/"+id_list[i]+"/?company="+this.company.id+"&marketplace="+this.marketplace.id).then((res)=>{
