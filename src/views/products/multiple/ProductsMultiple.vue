    ...
    <div id="root" class="root hd--expanded hd--sticky mn--sticky" :class="{ 'mn--max' : !collapse, 'mn--min' : collapse, }">
         ...
         <div class="content__header content__boxed overlapping">
             <div class="content__wrap">
                     ...
                     <h1 class="page-title mb-2">Prodotti Multipli</h1>
                     <h2 class="h5">Lista delle schede dei prodotti multipli</h2>
                     <p></p>
                         ...
                         <table v-if="products && !loading" class="table table-borderless table-striped table-hover table-sm">
                             ...
                             <tbody>
                                 <tr v-for="(product,index) in products" :key="index">
                                     ...
                                     <td v-show="fields['child'].show">
                                        <template v-if="'product' in product">
                                            <a :href="'/product/simple/?id='+product['product'].id+'&marketplace='+this.marketplace.id+'&company='+this.company.id">{{product['product']['sku']}}</a>/<b>{{product['qty']}}</b>
                                        </template>
                                     </td>
                                     ...
                                     <td><a class="btn" title="Vedi/Modifica" :href="'/product/multiple/?company='+this.company.id+'&marketplace='+this.marketplace.id+'&id='+product.id"><i class="bi bi-eye"></i></a></td>
                                 </tr>
                             </tbody>
                         </table>
                     </div>
         ...
         <Sidebar :collapse.sync="collapse" :company.sync="company" :companies.sync="companies" @update:company="(index) => changeCompany(index)" parent="products" child="multiple" @update:collapse="(collapse=false)"/>
     </div>
 </template>
 ...
 function initialState (){
   return {
             ...
             fields: {
                 ...
                 child:{name:"Prodotto/Quantità",show:true},
     ...
     methods:{
         async getProductsCategories(){
             for(let i=0;i<this.products.length;i++){
                 await this.axios.get("/api/products/categories/?company="+this.company.id+"&marketplace="+this.marketplace.id+"&multiple="+this.products[i].id).then((res)=>{
         ...
         async getProducts() {
             ...
             try{
                 const res = await this.axios.get("/api/products/multiple/?company="+this.company.id+"&marketplace="+this.marketplace.id+"&offset="+this.offset+"&limit="+this.limit+"&search="+this.search).then((res) => {
                     this.products=[]
                     for(var i=0; i<res.data.results.length; i++){
                         if(res.data.next){
         ...
         editProduct(key){
             this.$router.push("/product/multiple?company="+this.company.id+"&id="+key);         
         },
         shutdownProducts(){
             ...
             if(this.actionToDo=="duplicate"){
                 ...
                 for(var [key,product] of Object.entries(this.products)){
                     if(id_list.includes(product.id)){
                         ...
                         await this.axios.post("/api/products/multiple/?company="+this.company.id+"&marketplace="+this.actionMarket,data).then((res)=>{
                 ...
                 for(var i=0;i<id_list.length;i++){
                     await this.axios.delete("/api/products/multiple/"+id_list[i]+"/?company="+this.company.id+"&marketplace="+this.marketplace.id).then((res)=>{
