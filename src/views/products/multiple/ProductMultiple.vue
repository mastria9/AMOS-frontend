  ...
  <div id="root" class="root hd--expanded hd--sticky mn--sticky" :class="{ 'mn--max' : !collapse, 'mn--min' : collapse, }">
        ...
        <div class="content__header content__boxed overlapping">
            <div class="content__wrap">
                ...
                <h1 class="page-title mb-2">Prodotto Multiplo</h1>
        ...
        <div class="content__boxed" v-if="this.ready">
            <div class="content__wrap">
                ...
                <div class="row p-2 bg-light">
                    ...
                    <div class="col-4">
                        <h5>Associato a <a :href="'/product/simple/?id='+product['product'].id+'&marketplace='+this.marketplace.id+'&company='+this.company.id">{{product['product']['sku']}}</a>/<b>{{product['qty']}}</b></h5>
                    </div>
                    <div class="col-4 d-md-flex justify-content-md-end">
                        <a :href="'/products/multiple/?company='+this.company.id+'&marketplace='+this.marketplace.id" class="btn btn-primary">Tutti i prodotti</a>
                    </div>
                </div>
                <div class="row p-2 bg-light">
                    <div class="col-3">
                    ...
                    <div class="col-3">
                        <template v-if="this.category">
                          <b>Cambia associazione</b>
                          <select class="form-select" v-model="this.child.id">
                            <option v-for="(child,key) in this.category.simple" :key="key" :value="child.id" >{{child.sku}}</option>
                          </select>
                        </template>
                      </div>
                      <div class="col-2">
                        <b>Quantità nel pacco (minimo 2)</b>
                        <input class="form-control" type="number" min="2" v-model="this.child.qty">
                      </div>
                      <div class="col-1 m-auto ms-0 mb-0">
                        <button class="btn btn-warning" v-on:click="changeAssociation()">Cambia</button>                        
                      </div>
...
export default{
	...
	methods:{
        ...
        async getProductCategory(){
            const res = await this.axios.get("/api/products/categories/?company="+this.company.id+"&marketplace="+this.marketplace.id+"&multiple="+this.$route.query.id).then((res)=>{
                if(res.data.results.length>0){
        ...
        async getProduct(){
                ...
                const res = await this.axios.get("/api/products/multiple/"+this.$route.query.id+"/?company="+this.company.id+"&marketplace="+this.marketplace.id).then((res)=>{
                        ...
                        this.product.product=res.data.product;
        ...
        changeCompany(index){
            window.location.href='/products/multiple?company='+this.companies[key].id;
            
        },
        changeMarketplace(event){
            window.location.href='/products/multiple?company='+this.company.id+'&marketplace='+event.target.value
        },
        ...
        saveProduct(){
            ...
            this.axios.put("/api/products/multiple/"+this.product.id+"/?company="+this.company.id+"&marketplace="+this.marketplace.id,this.product).then((res)=>{
        ...
        copyFromProduct(){
            ...
            this.axios.put("/api/copy/from/multiple/?company="+this.company.id+"&marketplace="+this.marketplace.id+"&product="+this.product.id,data).then((res)=>{
                this.toast.success("Copia effettuata");                
            }).catch((error)=>{
                this.toast.error(error.response.data.detail)
            })
        ...
        changeAssociation(){
            ...
            product["child"]=this.child.id
            product["qty"]=this.child.qty
            
            this.axios.put("/api/products/multiple/"+this.product.id+"/?company="+this.company.id+"&marketplace="+this.marketplace.id,product).then((res)=>{
                this.toast.success("Associato");
                this.getProduct().then(this.getProductCategory)
            }).catch((error)=>{
                this.toast.error(error.response.data.detail)
            })
