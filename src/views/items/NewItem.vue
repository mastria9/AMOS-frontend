<template>
      ...
      <section class="content" id="content">
        <div class="content__header content__boxed overlapping">
          <div class="content__wrap">
            ...
            <h1 class="page-title mb-2">Nuovo articolo</h1>
            <h2 class="h5">Crea un nuovo articolo</h2>
            <p></p>
                ...
                <div class="row p-2 bg-light mt-2">
                    <div class="card col-12 row m-auto p-4">
                        <div class="card-header">
                            <h5 class="card-title">Crea articolo</h5>
                                ...
                                <div class="col-4">
                                    <table>
                                      <tr>
                                            <th>Magazzino</th>
                                            <td>
                                              <select class="form-select" v-model="this.warehouse">
                                                <option v-for="warehouse,key in this.warehouses" :value="warehouse">{{warehouse.code}} - {{warehouse.name}}</option>
                                              </select>
                                              </td>
                                        </tr>
                                        <tr>
                    ...
                    <div class="card-footer row">
                        ...
                        <div class="col-6 d-md-flex justify-content-md-end">
                            <button class="btn btn-primary" v-on:click="this.create()">Crea</button>
                        </div>
                    </div>
                </div>
              </div>
  ...
  export default {
    ...
    methods: {
      async init() {
        this.getCompanies().then(this.getWarehouses)
      },
      async getCompanies() {
        try {
      ...
      async create(){
        ...
        for(var i=0; i<this.files.length;i++){
            ...
            let res = await this.axios.post("/api/warehouse/files/?company="+this.company.id,formdata,{headers:{ 'X-CSRFToken':$cookies.get('csrftoken'), 'Content-Type': 'multipart/form-data','Content-Disposition':'attachment;filename='+this.files[i].name}}).then((res) => {
                ...
                this.item.files.push(res.data.id)
            })
            .catch((error) => {
            this.toast.error(error.response.data.detail);
            });
        ...
        await this.axios.post("/api/warehouse/items/?company="+this.company.id+'&warehouse='+this.warehouse.id,this.item).then((res) => {
           this.toast.success("Articolo creato");
           this.$router.push("/items/?company="+this.company.id+"&warehouse="+this.warehouse.id)
        })
        .catch((error) => {
         this.toast.error(error.response.data.detail);
        });
