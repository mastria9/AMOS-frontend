                    ...
                    <div class="mt-4 col-12 card" >
                      <div class="card-header">
                        ...
                        <div class="card-body m-auto" v-if="this.attribute">
                          ...
                          <hr/>
                          <button class="btn btn-success " v-on:click="save()">Salva</button><button class="float-right btn btn-danger " v-on:click="this.$router.push('/attributes/?company='+this.company.id)">Annulla</button>
                        </div>
                    </div>
                  </div>          
              </div>
...
export default{
	...
	methods:{
        ...
        async init(){
            this.getCompanies().then(this.getAttribute)
        },

        async getAttribute(){
          try{
            if (this.$route.query.id){
                    const res = await this.axios.get("/api/products/attributes/"+this.$route.query.id+"/?company="+this.company.id).then((res)=>{
      ...
      save(){
          ...
          this.axios.put("/api/products/attributes/"+this.attribute.id+"/?company="+this.company.id,this.attribute).then((res)=>{
            window.location.href='/attributes/?company='+this.company.id;
