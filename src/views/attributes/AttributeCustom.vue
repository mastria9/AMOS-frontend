    ...
    <div id="root" class="root hd--expanded hd--sticky mn--sticky" :class="{ 'mn--max' : !collapse, 'mn--min' : collapse, }">
        ...
        <div class="content__header content__boxed overlapping">
            <div class="content__wrap">
                    ...
                    <h1 class="page-title mb-2">Modifica attributo</h1>
                    <h2 class="h5">Modifica questo attributo</h2>
                    <p></p>
                  ...
                  <div class="col-12 row">
                    <div class="mt-4 col-12 card" >
                      <div class="card-header">
                        ...
                        <div class="card-body m-auto" v-if="this.attribute">
                          ...
                          <button class="btn btn-primary " v-on:click="this.save()">Salva</button>
                          <button class="btn btn-warning " v-on:click="this.$router.push('/attributes/?company='+this.company.id)">Annulla</button>
                          <button class="btn btn-danger " v-on:click="this.del()">Elimina</button>
                        </div>
                    </div>
                  </div>          
              </div>
...
export default{
	...
	methods:{
        ...
        async init(){
            this.getCompanies().then(this.getMarketplaces).then(this.getAttribute)
        },

        async getAttribute(){
          try{
            if (this.$route.query.id){
                    const res = await this.axios.get("/api/products/custom_attributes/"+this.$route.query.id+"/?company="+this.company.id).then((res)=>{
                        
                        if(res.data.length==0){
                            this.toast.warning("Nessun attributo");
                        }
                        else{
      ...
      save(){
          ...
          this.axios.put("/api/products/custom_attributes/"+this.attribute.id+"/?company="+this.company.id+"&marketplace="+this.attribute.marketplace,this.attribute).then((res)=>{
...
this.axios.delete("/api/products/custom_attributes/"+this.attribute.id+"/?company="+this.company.id+"&marketplace="+this.attribute.marketplace).then((res)=>{
  window.location.href='/attributes/?company='+this.company.id;
